...
function renderTrend(trend) {
  ...
  if (!trend.length) {
    host.innerHTML = '<div class="sub">No data in range.</div>';
    return;
  }
  ...
  // Cap visible bar width so a single-day range doesn't render one huge block.
  ...
  pick.forEach(i => {
    ...
    t.textContent = trend[i].date.slice(5); // MM-DD
    s.appendChild(t);
  });
...
function renderHeatmap(hm) {
  ...
  // log-ish bucketing: 0, <=5%, <=20%, <=50%, >50%
  function bucket(v) {
    if (!v) return 0;
    if (max <= 0) return 0;
    const r = v / max;
  ...
  for (let w = 0; w < weeks; w++) {
    for (let row = 0; row < 7; row++) {
      ...
      const b = bucket(v);
      const fill = `var(--grid-${b})`;
      const rect = svg('rect', {
        x: w * (cell + gap), y: row * (cell + gap),
        width: cell, height: cell, rx: 2, ry: 2, fill
      });
      rect.addEventListener('mousemove', (e) => showTip(e, `${iso} · ${fmtInt(v)} 有效 token`));
      rect.addEventListener('mouseleave', hideTip);
      g.appendChild(rect);

      // Place the month label over the week where the month first appears.
  ...
  for (const ml of monthLabels) {
    const t = svg('text', {
      x: ml.w * (cell + gap), y: 10, class: 'axis'
    });
...
function renderFooter(d) {
  const lines = [];
  lines.push(`codex_home: ${d.codex_home || '(unknown)'}`);
...
async function load() {
  ...
  try {
    ...
    const r = await fetch(`/api/stats?range=${encodeURIComponent(state.range)}`);
    if (!r.ok) throw new Error('HTTP ' + r.status);
    const d = await r.json();
    state.cache[state.range] = d;
    renderDashboard(d);
  } catch (e) {
    $('sub').innerHTML = `<span class="err">加载失败：${e.message}</span>`;
  }
}
...
async function refresh() {
  ...
  $('sub').textContent = '刷新中...';
  try {
    const r = await fetch(`/api/stats?range=${encodeURIComponent(state.range)}&refresh=1`);
    if (!r.ok) throw new Error('HTTP ' + r.status);
    const d = await r.json();
    state.cache[state.range] = d;
    renderDashboard(d);
  } catch (e) {
    $('sub').innerHTML = `<span class="err">刷新失败：${e.message}</span>`;
  }
}
...
function bind() {
  ...
  window.addEventListener('resize', () => {
    // Only the trend chart depends on available width.
    const last = window.__lastTrend;
